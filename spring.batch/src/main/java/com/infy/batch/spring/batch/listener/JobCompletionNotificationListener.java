package com.infy.batch.spring.batch.listener;
...
public class JobCompletionNotificationListener extends JobExecutionListenerSupport {
	...
	public void afterJob(JobExecution jobExecution) {
		if(jobExecution.getStatus() == BatchStatus.COMPLETED) {
			...
			List<Client> results2 = jdbcTemplate.query("SELECT client_name FROM client", new RowMapper<Client>() {
				@Override
				public Client mapRow(ResultSet rs, int row) throws SQLException {
					return new Client(rs.getString(1));
				}
			});
